    ...
    methods: {
        ...
        handleSearch: function () {
            ...
            if (keywords.trim(" ") !== "") {
                ...
                listen1_search("netease", keywords).then(data => {

                    for (let v of data.result)
                        r_ne.push(v);

                    listen1_search("qq", keywords).then(data => {

                        for (let v of data.result)
                            r_qq.push(v);

                        listen1_search("xiami", keywords).then(data => {
                ...
                // listen1_search(this.temp_source, this.temp_search_title).then(data => {
                //     this.temp_search_result = data.result;
                //     this.tableLoading = false;
                // }).catch(err => {
                //     this.tableLoading = false;
                //     alert(err);
...
<p>歌手: {{ track.artist }} ({{ track.artist_id }}) 专辑: {{ track.album }} ({{ track.album_id }})</p>
  ...
  <el-input placeholder="请输入歌曲名，歌手或专辑" v-model="temp_search_title" clearable @keyup.enter.native="handleSearch" class="select-source" >
    ...
    <el-table ref="singleTable"
     ...
     style="width: 100%">
        ...
        <el-table-column fixed="right" label="操作">
            ...
            <el-button type="primary" size="mini" v-if="isadd"  icon="el-icon-success" @click="emitAdded(scope.row)">加入歌单</el-button>
            <el-button type="primary" size="mini" v-if="!isadd" icon="el-icon-circle-plus-outline" @click="emitSelected(scope.row)">选择</el-button>
